...
public class RenameDocumentInTestFolder {
	...
	public static void go(TestSetting setting) {
		...
		try {
			Document toDeleteDoc = (Document) session.getObjectByPath("/Test/a.txt");
		...
		try {
			Document toDeleteDoc = (Document) session.getObjectByPath("/Test/b.txt");
			Tool.printAndLog("Dokument vorhanden + wird gelöscht.");
			toDeleteDoc.delete();
			Tool.printAndLog("Dokument erfolgreich gelöscht.");
		} catch (Exception e) {
		...
		Document doc2Create = testFolder.createDocument(props, null, null);

		Tool.printAndLog("Name Of the Document " + doc2Create.getName());
		Tool.printAndLog("Path Of the Document " + doc2Create.getPaths().get(0));

		// Dokument finden
		Document document = (Document) session.getObjectByPath("/Test/a.txt");
